...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// /api/consult-chat — AI재무진단 텍스트 상담 (v3.16.4)
// v3.16.4: 고수님 Vapi 원본 프롬프트 100% 적용 (음성과 완전 동일)
...
④ "부동산은 자가이신가요? 금액은 얼마이신가요?"
⑤ "그밖의 부동산이 있으신가요?"
⑥ "신용대출이 있으신가요?"
⑦ "주택담보대출은요?"
부자지수 = (순자산X10 / (나이 × 월수입X12))
...
- 한 응답에 질문은 오직 1개 (단 5단계 오프닝의 경우 "집을 그릴 때 어디서부터" 후 고객 답변 기다림, 그다음 "재무도 똑같습니다..." 설명 후 "은퇴는 몇 세로?" 질문)
...
app.post('/api/consult-chat', async (req, res) => {
  try {
    ...
    console.log('[/api/consult-chat v3.16.4] ' + (userName || 'guest') + ': ' + message.substring(0, 30) + '... -> ' + aiMessage.substring(0, 30) + '...');
